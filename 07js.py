...
def sum_digits(n):
    ...
    126
    """
    if n < 10:  # base case, without recursive call.
        return n   # single digit
    else:
        all_but_last, last = split(n) # recursive cases 
...
def fact(n):  # env diagram is shown in the lecture.
    if n == 0:
        return 1
    else:
        return n * fact(n-1)
...
def luhn_sum(n):
    if n< 10:
        ...
        return uhn_sum_double(all_but_last) + last 

def lun_sum_double(n):
    all_but_last, last  = split(n)
